...
function applyConnectionSettings (api, options) {
  api.requestModule = api.requestModule.defaults({
    headers: {
      'user-agent': `MEGAJS-CLI/${packageInfo.version} (+${packageInfo.homepage})`
    }
  })
...
function createFileList (topFile, basename, options) {
  ...
  return (options.andSelf ? [topFile] : []).concat(topFile.children).map(file => {
    ...
    const filename = file.name
      ...
      : '[encrypted filename]'
    const filePath = file === topFile ? basename
      : basename ? path.posix.join(basename, filename)
        : filename
...
function downloadFileTo (file, target, options = {}, callback = defaultDownloadCallback) {
  ...
  function ensureCallback (err) {
    ...
    let downloadStream = storage.api
      .download({
        start: startByte,
