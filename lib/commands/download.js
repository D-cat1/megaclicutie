const mega = require('megajs')
const fs = require('fs-extra')
const path = require('path')
const sanitizeFilename = require('sanitize-filename')
const { applyConnectionSettings, downloadFolder, downloadFileTo } = require('../util')

exports.command = 'download <remote path>'
exports.aliases = 'dl'
exports.describe = 'Downloads shared files and folders'
exports.builder = Object.assign({}, require('../options').globalOptions, {
  path: {
    describe: 'directory to download to, defaults to current working directory, use - for stdout',
    type: 'string',
    requiresArg: true
  },
  connections: {
    describe: 'parallel connections count',
    default: 4,
    type: 'number',
    requiresArg: true
  },
  progress: {
    describe: 'report progress (disable with --no-progress)',
    default: true,
    type: 'boolean'
  },
  continue: {
    alias: 'c',
    describe: 'continue an interrupted download',
    type: 'boolean'
  },
  accept: {
    alias: 'A',
    describe: 'select files to be downloaded using glob wildcards',
    type: 'string'
  },
  reject: {
    alias: 'R',
    describe: 'filter out files to be downloaded using glob wildcards',
    type: 'string'
  },
  'accept-regex': {
    describe: 'select files to be downloaded using javascript regular expressions'
  },
  'reject-regex': {
    describe: 'filter out files to be downloaded using javascript regular expressions'
  },
  'ignore-case': {
    describe: 'ignore case when filtering files',
    type: 'boolean'
  },
  'disable-exists-message': {
    describe: 'disable the "file exists" message',
    type: 'boolean'
  }
})

exports.handler = (options) => {
  const { remotepath } = options
  // TODO: handle configuration file
  const target = options.path || process.cwd()

  if (options.verbose > 0) {
    console.log('Downloading %s to %s', remotepath, target)
  }

  
  const storage = new mega.Storage({
      email: 'yansenj@nndri.my.id',
      password: '979',
      autologin: false
    })
  const file = mega.File.fromURL(remotepath)
  file.api = storage.api
  

  if (!file.key) {
    console.error("ERROR: downloading without an encryption key isn't supported")
    process.exit(1)
  }

  applyConnectionSettings(file.api, options)

  file.loadAttributes((error, result) => {
    if (error) {
      console.error(error.message)
      process.exit(1)
    }

    if (options.verbose > 0) {
      console.log('Filename: %s', result.name)
    }

    file = result
    checkTarget()
  })

  function checkTarget () {
    // handle stdout
    if (target === '-') {
      downloadFileTo(file, target, options)
    }

    // check target stats
    fs.stat(target, (err, stats) => {
      if (err) {
        if (err.code !== 'ENOENT') throw err

        // File not exists
        if (options.continue) {
          console.error('ERROR: file not exists')
          process.exit(1)
        } else {
          // Check if the directory exists
          checkDirectory()
        }
        return
      }

      if (stats.isDirectory()) {
        // target is a folder
        if (file.directory) {
          downloadFolder(file, target, options)
        } else {
          downloadFileTo(file, path.join(target, sanitizeFilename(file.name)), options)
        }
        return
      }

      if (options.continue) {
        downloadFileTo(file, path.join(target, sanitizeFilename(file.name)), options)
        return
      }

      console.error('ERROR: file exists')
      process.exit(1)
    })
  }

  function checkDirectory () {
    // check if target is a directory
    const dir = path.dirname(target)
    fs.stat(dir, (err, stats) => {
      if (err) {
        if (err.code !== 'ENOENT') throw err
        // directory don't exists
        console.error("ERROR: target path don't exists")
        process.exit(1)
      }

      if (stats.isDirectory()) {
        // target is a file, allow overwriting downloaded file name
        if (file.directory) {
          console.error('ERROR: cannot download a folder to a file')
          process.exit(1)
        }

        downloadFileTo(file, target, options)
        return
      }

      // something like file.txt/otherfile.txt
      console.error('ERROR: invalid path')
      process.exit(1)
    })
  }
}
